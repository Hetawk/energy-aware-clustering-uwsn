...
class Simulation:
    def __init__(self, sensorList, relayList, Fin_Conn_S_R, Fin_Conn_R_R, membership_values, cluster_heads=None, rounds=10):
        ...
        if self.membership_values is not None:
            # Ensure membership_values is in correct shape (n_clusters x n_sensors)
            if isinstance(self.membership_values, np.ndarray):
                if len(self.membership_values.shape) != 2:
                    self.membership_values = self.membership_values.reshape(-1, len(sensorList))
            else:
                print("Warning: membership_values not properly initialized")
                self.membership_values = None
        self.cluster_heads = cluster_heads
        self.rounds = rounds
        self.init_energy = 0.0005 * len(sensorList) * len(relayList)  # Add this line
        self.nw_e_s = None  # Add this line
    ...
    def state_matrix(self):
        state_s = [[0 for j in range(len(self.relayList))] for i in range(len(self.sensorList))]
        ...
        if not oil_affected_relays:
            for j in range(len(s_counter)):
                ...
                for i in range(len(self.sensorList)):
                    if self.Fin_Conn_S_R[i][j] == 1:
                        ...
                        if self.membership_values is not None and j < len(self.membership_values):
                            ...
                            membership_score = float(self.membership_values[j][i])
                            energy_score = float(sum(self.nw_e_s[i])) / self.init_energy
                ...
                if sensors_in_cluster:
                    ...
                    energy_ratio = total_energy / (self.init_energy * len(self.sensorList))
                    ...
                    active_count = max(1, int(len(sensors_in_cluster) * active_ratio))
                    
                    # Sort and activate top sensors
                    sorted_pairs = sorted(zip(scores, sensors_in_cluster), reverse=True)
                    for i, (_, sensor_idx) in enumerate(sorted_pairs):
                        state_s[sensor_idx][j] = 1 if i < active_count else 0
                    
                    srpfile.write(f"Round {round}: Relay {j} - {active_count}/{len(sensors_in_cluster)} sensors active\n")
        else:
    ...
    def simu_network(self, nw_e_s, nw_e_r, state_s):
        ...
        # Update file paths
        file = open("results/network/percentage_network.txt", "w")
        srpfile = open("results/network/srp_output.txt", "w")
        file.write("sensors: " + str(len(self.sensorList)) + "\n")
        file.write("Relays: " + str(len(self.relayList)) + "\n")
        file.write("\n")
        total_energy = 0
        round = 0
        dead_s = 0
        ...
        while round < self.rounds:  # Limit the number of rounds for testing
            consumed_round_energy = 0.0  # Initialize as float
                            ...
                            try:
                                if self.membership_values is not None:
                                    ...
                                    if j < len(self.membership_values[0]):
                                        membership_val = float(self.membership_values[cluster_idx][j])
                                        # Reduce energy consumption for high membership nodes
                                        energy_used = base_energy * (0.8 + 0.2 * membership_val)
                                    else:
            ...
            file.write("dead relays: " + str(dead_r) + "\n")
            file.write("Dead Network pc: " + str(dead_nw_pc) + " % \n")
            file.write("Dead Sensor pc: " + str(dead_s_pc) + " % \n")
            file.write("Dead relays pc: " + str(dead_r_pc) + " % \n")
            if round == 5:
            ...
            if dead_s_pc > 90 or dead_r_pc > 90:
                break
            self.srp_toggler(state_s, s_counter, PH_list_sensors, round, srpfile)
        ...
        # Save additional results to CSV
        self.save_to_csv("results/energy/final_energy_s.csv", ["Sensor", "Energy"],
                         [[i, sum(row)] for i, row in enumerate(nw_e_s)])
        self.save_to_csv("results/energy/final_energy_r.csv", ["Relay", "Energy"],
                         [[i, sum(row)] for i, row in enumerate(nw_e_r)])
        self.save_to_csv("results/state/state_s.csv", ["Sensor", "State"], 
                         [[i, sum(row)] for i, row in enumerate(state_s)])

        return nw_e_s, init_energy_copy  # Return both final and initial energy matrices