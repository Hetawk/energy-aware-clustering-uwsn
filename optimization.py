...
class Optimization:
    ...
    def populate_sensor_relay_lists(self, sen):
        for i in range(sen):
            self.sensorList.append((round(random.uniform(0.0, self.width), 6), round(random.uniform(0.0, self.width), 6)))
        row = 0
        col = 0
    ...
    def connection_s_r(self):
        Neighbor_Sensor_Relay = [[[0] for i in range(len(self.relayList))] for j in range(len(self.sensorList))]
    ...
    def connection_r_r(self):
        Neighbor_Relay_Relay = [[[0] for i in range(len(self.relayList))] for j in range(len(self.relayList))]
    ...
    def link_s_r(self):
        bandwidth = 100.0
        Linkflow_Sensor_Relay = [[[0] for i in range(len(self.relayList))] for j in range(len(self.sensorList))]
        for i in range(len(self.sensorList)):
            for j in range(len(self.relayList)):
    ...
    def link_r_r(self):
        bandwidth = 200.0
        Linkflow_Relay_Relay = [[[0] for i in range(len(self.relayList))] for j in range(len(self.relayList))]
        for i in range(len(self.relayList)):
            for j in range(len(self.relayList)):
                if i != j:
    ...
    def energy_s_r(self):
        bandwidth = 100.0
        Energy_Sensor_Relay = [[[0] for i in range(len(self.relayList))] for j in range(len(self.sensorList))]
        E_radio_S = 50.0 * (10 ** (-9))
    ...
    def energy_r_r(self):
        bandwidth = 200.0
        Energy_Relay_Relay = [[[0] for i in range(len(self.relayList))] for j in range(len(self.relayList))]
        E_radio_R = 100.0 * (10 ** (-9))
        Transmit_amplifier = 100.0 * (10 ** (-12))
        for i in range(len(self.relayList)):
            for j in range(len(self.relayList)):
                dist = self.distance(abs(self.relayList[i][0] - self.relayList[j][0]), abs(self.relayList[i][1] - self.relayList[j][1]))
                energy_relay_tx = float(bandwidth * (E_radio_R + (Transmit_amplifier * (dist ** 2))))
                energy_relay_rx = float(bandwidth * E_radio_R)
    ...
    def apply_clustering(self):
        ...
        try:
            ...
            self.membership_values = u  # Store membership values (n_clusters x n_sensors)
            
            # Ensure membership values are properly shaped
            if self.membership_values.shape[1] != len(self.sensorList):
                print("Warning: Reshaping membership values")
                self.membership_values = self.membership_values.reshape(-1, len(self.sensorList))
            
            # Update sensor list with cluster info
            clustered_sensor_data = []
            for i, (sensor, label) in enumerate(zip(sensor_data, self.cluster_labels)):
                membership = u[:, i]  # Get membership values for this sensor
                clustered_sensor_data.append(np.concatenate([sensor, [label], membership]))
            
            self.sensorList = clustered_sensor_data
    ...
    def select_cluster_heads(self):
        ...
        try:
            ...
            # Calculate normalized energy values and distances
            ...
            for i in range(3):
                ...
                if np.any(mask):
                    points = np.array([s[:2] for s, m in zip(self.sensorList, mask) if m])
            ...
            for cluster in range(3):
                ...
                if cluster_nodes:
                    ...
                    # Update energy ratios to encourage balanced selection
                    selected_idx = self.sensorList.index(cluster_nodes[max_score_idx])
                    energy_ratios[selected_idx] *= 0.8  # Reduce probability of reselection
    ...
    def solve_problem(self):
        ...
        Var_S_R = [LpVariable(str('SR' + str(i) + "_" + str(j)), 0, 1, LpInteger) for i in range(len(self.sensorList)) for j in range(len(self.relayList))]
        Var_R_R = [LpVariable(str('RR' + str(i) + "_" + str(j)), 0, 1, LpInteger) for i in range(len(self.relayList)) for j in range(len(self.relayList))]
        Bool_Var = [LpVariable(str('B' + str(i)), 0, 1, LpInteger) for i in range(len(self.relayList))]
